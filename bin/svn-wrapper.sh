...
# Whether we're diffing interactively or not
diff_interactive=no
...
if test x"$COLORDIFF" = xmissing; then
  if test -t 1; then
    COLORDIFF=`find_progs diffc colordiff diff`
  else
    COLORDIFF=$DIFF
  fi
fi

# If `less' is the $PAGER, turn on some useful options: -R to let it interpret
# some terminal codes (turns on colors and accentuated characters), -i to
# allow case-insensitive search if the pattern doesn't contain upper case
# letters.
case $PAGER in #(
  less | 'less '*) PAGER="$PAGER -R -i";;
esac
...
vi: ft=diff:noet:tw=76:" >>"$tmp_log"
 ...
 ${lgreen}can't$std:"
  ...
  case $chlog_entry in
    *@FIXME* | *[tT]ype' '[yY]our' '[nN]ame*)
      warn 'Suspicious ChangeLog entry:'
      echo "$chlog_entry" | grep -i '@FIXME\|[tT]ype [yY]our [nN]ame'
      yesno 'Continue anyway?' || return 1
      ;;
  esac
  ...
  case $mail_from in
    *@FIXME* | *[tT]ype' '[yY]our' '[nN]ame*)
      warn "Suspicious the From field: $mail_from"
      yesno 'Continue anyway?' || return 1
      ;;
  esac
...
svn_diff()
{
  ...
  if $git_mode; then
    ...
    # Extra args for diff (with SVN only, Git has a decent diff).
    : ${svn_diffarg=-u}

    # If $PAGER contains -R, it most likely understand colors and stuff, so we
    # can use it to show the colored diff.
    diff_cmd_='$SVN diff --no-diff-deleted --diff-cmd $DIFF -x $svn_diffarg "$@"'
    case $diff_interactive:$PAGER in #(
      yes:*-R*) eval "$diff_cmd_" | $PAGER;;
      *) eval "$diff_cmd_"
    esac
  fi
}
...
svn_help()
{
  if [ $# -eq 0 ]; then
    ...
    rv=$?
    echo '
Additional commands provided by svn-wrapper:
  diffstat (ds)
  diffw (dw)
  ignore
  mail
